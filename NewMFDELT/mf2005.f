...
C-------ASSIGN VERSION NUMBER AND DATE
      ...
      CHARACTER*10 MFVNAM
      PARAMETER (VERSION='1.9.00SeaWat Beta6/28/2010')
...
c      IF(IUNIT(59).GT.0) CALL PCGN2AR(IUNIT(59),IFREFM,MXITER,IGRID)
...
C  VDF  compute WATLEV
            IF(IUNIT(1).GT.0) CALL GWF2BCF7FM(KKITER,KKSTP,
     1                               KKPER,IGRID)
            IF(IUNIT(23).GT.0) THEN
              CALL GWF2LPF7FMCD(KKITER,KKSTP,KKPER,IGRID)
              IF(IUNIT(60).EQ.0) THEN
                CALL GWF2LPF7FMST(KKPER,IGRID)
              ELSE
C VDF conductance adjustment
C VDF storage
              END IF
            END IF
...
c            END IF
     ...
     6                         BIGHEADCHG,HNEWLAST)
            ENDIF
c            IF (IUNIT(59).GT.0) THEN
c              CALL PCGN2AP(HNEW,RHS,CR,CC,CV,HCOF,IBOUND,
c     1              KKITER,KKSTP,KKPER,ICNVG,HNOFLO,IGRID)
c            ENDIF      
...
C7C4----CALCULATE BUDGET TERMS. SAVE CELL-BY-CELL FLOW TERMS.
          MSUM = 1
C  VDFBASBD -- compute WATLEV from HNEW
          IF (IUNIT(1).GT.0) THEN
   ...
   37       CONTINUE
          ...
          IF(IUNIT(23).GT.0) THEN
            ...
            ELSE
C             CALL VDF2LPF7BDS(KKSTP,KKPER,IGRID)
            END IF
            IBDRET=0
            IF(IUNIT(60).GT.0) IBDRET=0
            IC1=1
            IC2=NCOL
            IR1=1
            IR2=NROW
            ...
            CALL GWF2LPF7BDADJ(1,IBDRET,IC1,IC2,IR1,IR2,IL1,IL2,
     1                        IPAKCB,IBD,IGRID)
C  VDFLPFBD -- Right face mass and CH right face
            CALL UBUDBUFSAV(KKSTP,KKPER,'FLOW RIGHT FACE ',IPAKCB,IBD)

            CALL GWF2LPF7BDADJ(2,IBDRET,IC1,IC2,IR1,IR2,IL1,IL2,
     1                        IPAKCB,IBD,IGRID)
C  VDFLPFBD -- Front face mass and CH front face
            CALL UBUDBUFSAV(KKSTP,KKPER,'FLOW FRONT FACE ',IPAKCB,IBD)

            CALL GWF2LPF7BDADJ(3,IBDRET,IC1,IC2,IR1,IR2,IL1,IL2,
     1                        IPAKCB,IBD,IGRID)
C  VDFLPFBD -- Lower face mass and CH lower face
            CALL UBUDBUFSAV(KKSTP,KKPER,'FLOW LOWER FACE ',IPAKCB,IBD)

            IF(IUNIT(60).EQ.0) THEN
               CALL GWF2LPF7BDCH(KKSTP,KKPER,IGRID)
            ELSE
C               CALL VDF2LPF7BDCH(KKSTP,KKPER,IGRID)
            END IF
          ENDIF
...
C
C-----END OF TIME STEP (KSTP) LOOP.  SKIP OUT OF THIS LOOP IF LASTTS
C-----FLAG IS SET.
...
c      IF(IUNIT(59).GT.0) CALL PCGN2DA(IGRID)
