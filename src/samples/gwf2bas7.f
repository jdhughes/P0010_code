      MODULE GLOBAL
        ...
        INTEGER, SAVE, POINTER    ::IFREFM,NODES,IOUT,MXITER,LASTTS
        INTEGER, SAVE,    DIMENSION(:),     POINTER ::IUNIT(:)
        DOUBLE PRECISION, SAVE, DIMENSION(:,:,:), POINTER ::HNEW
        DOUBLE PRECISION, SAVE, DIMENSION(:,:,:), POINTER ::WATLEV
        ...
        INTEGER, SAVE,    DIMENSION(:),     POINTER ::ISTPADJ
        ...
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER ::WATLEVOLD
      ...
      TYPE GLOBALTYPE
        ...
        INTEGER,POINTER    :: IFREFM,NODES,IOUT,MXITER,LASTTS
        INTEGER,    DIMENSION(:),     POINTER ::IUNIT
        DOUBLE PRECISION, DIMENSION(:,:,:), POINTER ::HNEW
        DOUBLE PRECISION, DIMENSION(:,:,:), POINTER ::WATLEV
        ...
        INTEGER,    DIMENSION(:),     POINTER ::ISTPADJ
        ...
        REAL,       DIMENSION(:,:,:), POINTER ::WATLEVOLD
...
C  this file to use the IMPLICIT NONE statement.
      ...
      MODULE GWFBASMODULE
        ...
        INTEGER, SAVE, POINTER  ::IPRTIM,IPEROC,ITSOC,ICHFLG,MASSFL
        INTEGER, SAVE, POINTER  ::IDDREF,IDDREFNEW,IOCREP
      ...
      TYPE GWFBASTYPE
        ...
        INTEGER, POINTER  ::IPRTIM,IPEROC,ITSOC,ICHFLG,MASSFL
        INTEGER, POINTER  ::IDDREF,IDDREFNEW,IOCREP
...
C     ------------------------------------------------------------------
     ...
     5                     DDREF,WATLEV,WATLEVOLD,ISTPADJ,LASTTS
     ...
     3                      IDDREF,IDDREFNEW,IOCREP,DELT,PERTIM,TOTIM,
     4                      HNOFLO,HDRY,CHEDFM,CDDNFM,CBOUFM,VBVL,VBNM,
     5                      MASSFL
C
...
C1------grids to be defined.
      ...
      ALLOCATE(IXSEC,INBAS,IFREFM,NODES,IOUT,MXITER,LASTTS)
      MXITER=1
...
C
     ...
     1         ICHFLG,MASSFL,IDDREF,IDDREFNEW,IOCREP)
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:IOUT,PERLEN,NSTP,TSMULT,LASTTS
...
C
C6------CLEAR FLAG THAT INDICATES LAST TIME STEP OF STRESS PERIOD
      LASTTS=0
C
C7------RETURN
      RETURN
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,TSMULT,HNEW,HOLD,NSTP,LASTTS
...
C
C4------SET FLAG IF LAST TIME STEP OF STRESS PERIOD
      IF(KSTP.EQ.NSTP(KPER)) LASTTS=1
C
C5------RETURN
      RETURN
      END
...
C     ------------------------------------------------------------------
     ...
     1                      MSUM,VBVL,VBNM,IDDREF,MASSFL
C     ------------------------------------------------------------------
C
      CALL SGWF2BAS7PNT(IGRID)
C
...
C4------PRINT TOTAL BUDGET IF REQUESTED
      ...
      CALL SGWF2BAS7V(MSUM,VBNM,VBVL,KSTP,KPER,IOUT,MASSFL)
      IPFLG=1
C
...
C14-----TIME STEP MULTIPLIER, AND STEADY-STATE FLAG..
  ...
  162    FORMAT(' SSFLAG MUST BE EITHER "SS" OR "TR"',
     1      ' -- STOP EXECUTION (SGWF2BAS7ARDIS)')
         CALL USTOP(' ')
      END IF
      WRITE (IOUT,163) N,PERLEN(N),NSTP(N),STPFLG,TSMULT(N),
     1                 LINE(ISTART:ISTOP)
  163 FORMAT(1X,I8,1PG21.7,I6,1X,A,0PF15.3,A11)
C
...
C6------RETURN
      RETURN
      END
      SUBROUTINE SGWF2BAS7V(MSUM,VBNM,VBVL,KSTP,KPER,IOUT,MASSFL)
C     ******************************************************************
...
C
  ...
  266 FORMAT(1X,/5X,'CUMULATIVE MASS   ',6X,'M**3',7X
     1,'RATES FOR THIS TIME STEP',6X,'M**3/T'/5X,18('-'),17X,24('-')
     2//11X,'IN:',38X,'IN:'/11X,'---',38X,'---')
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,       ONLY: LASTTS
      USE GWFBASMODULE, ONLY: IOFLG,IHDDFL,IBUDFL,ICBCFL,IPEROC,
     1                        ITSOC,IBDOPT,IDDREF,IDDREFNEW,IOCREP
C
      CHARACTER*200 LINE
C     ------------------------------------------------------------------
C  Special ALL time steps category
      IF(IOCREP.EQ.1) THEN
         IPEROC=KPER
         ITSOC=KSTP
         WRITE(IOUT,*) 'Repeating Output Control'
         RETURN
      END IF
      IF(IPEROC.EQ.0 .AND. ITSOC.EQ.0) THEN
         IPEROC=KPER
         ITSOC=KSTP
         IOCREP=1
         WRITE(IOUT,*)
     1       'Output Control will be the same for all time steps'
      END IF
C
C  Special End of Period category
      IF(KPER.EQ.IPEROC) THEN
         IF(ITSOC.EQ.0) THEN
            ITSOC=9999998
            WRITE(IOUT,*)
     1    'Output control is specified for the end of the stress period'
         END IF
         IF(ITSOC.EQ.9999998 .AND. LASTTS.NE.0) ITSOC=KSTP
      END IF
C
C1------ERROR IF OUTPUT CONTROL TIME STEP PRECEDES CURRENT SIMULATION
C1------TIME STEP.
 3    IF((IPEROC.LT.KPER).OR.(IPEROC.EQ.KPER .AND. ITSOC.LT.KSTP)) THEN
    ...
    5    FORMAT(1X,/1X,'OUTPUT CONTROL WAS SPECIFIED FOR A NONEXISTENT',
     ...
     5   1X,'SKIPPING TO THE NEXT OUTPUT CONTROL TIME')
 7       READ(INOC,'(A)',END=1000) LINE
         IF(LINE.EQ.' ') GO TO 7
         LLOC=1
         CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,INOC)
...
C
        ...
        DEALLOCATE(GLOBALDAT(IGRID)%LASTTS)
C
        ...
        DEALLOCATE(GLOBALDAT(IGRID)%ISTPADJ)
...
C
        ...
        ISTPADJ=>GLOBALDAT(IGRID)%ISTPADJ
        ...
        WATLEV=>GLOBALDAT(IGRID)%WATLEV
        HOLD=>GLOBALDAT(IGRID)%HOLD
        WATLEVOLD=>GLOBALDAT(IGRID)%WATLEVOLD
...
C
        ...
        GLOBALDAT(IGRID)%ISTPADJ=>ISTPADJ
        ...
        GLOBALDAT(IGRID)%WATLEV=>WATLEV
        GLOBALDAT(IGRID)%HOLD=>HOLD
        GLOBALDAT(IGRID)%WATLEVOLD=>WATLEVOLD
